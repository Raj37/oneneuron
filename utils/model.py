...
class Perceptron:
    def __init__(self, eta, epochs):
        ...
        print(f"initialize weights before training: \n{self.weights}")
        self.eta = eta
        self.epochs = epochs

    def activationFunction(self, inputs, weights):
        z = np.dot(inputs, weights) ## z = W*X
        return np.where(z > 0, 1, 0)  ## condition if-else statement

    def fit(self, X, y):
        self.X = X
        self.y = y

        X_with_bias = np.c_[self.X, -np.ones((len(self.X), 1))]  ## comcatenation
        print(f"X with bias: \n{X_with_bias}")
